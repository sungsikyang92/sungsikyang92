...
사진이 없으면 임시사진, 사진이 업로드되면 사진번호중 제일 큰것이 대표사진, 다시 사진이 없으면 임시사진으로 떠야한다. 그러나 사진이 업로되고도, 임시사진이 지속적으로 뜨는 문제가 발생하였다.

*   원인은?

우선 Mapper에서의 문제였다. 현재의 쿼리문으로는 사진넘버를 가져올때 최솟값을 가져온다. 최댓값을 가져와야 한다.

*   해결방법

결국 문제는 쿼리문과 나의 무지함에서 온 실수였다.

처음에 게시글이 작성되었지만 사진이 없을때, 임시사진을 넣기위해서 더미데이터사진을 집어넣었었다.

이것으로 임시사진을 출력할 수 있었으나, 사진이 업로드된 후에도 더미데이터가 남아있다는 문제가 있었다.

결국 임시사진을 input해주는 일련의 과정을 없애고, jsp에서 게시글은 jstl로 그리고 사진은 ajax로 불러왔다.

사진이 없을경우 jstl에서 대체사진으로 출력해주고, 사진이 있으면 ajax로 사진을 불러오게끔 하였다.
